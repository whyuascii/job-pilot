...
export const addExperience = createServerFn({ method: 'POST' }).validator(
  (data: {
    company: string;
    title: string;
    location?: string;
...
export const updateExperience = createServerFn({ method: 'POST' }).validator(
  (data: {
    experienceId: string;
    company?: string;
    title?: string;
...
export const deleteExperience = createServerFn({ method: 'POST' }).validator(
  (data: { experienceId: string }) => data,
).handler(async ({ data }) => {
  const ctx = await getTenantContext();
  const candidate = await getCurrentCandidate(ctx);
