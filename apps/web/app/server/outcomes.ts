...
export const listOutcomes = createServerFn({ method: 'POST' }).validator(
...
export const addOutcome = createServerFn({ method: 'POST' }).validator(
  (data: { applicationId: string; stage: string; notes?: string }) => data,
).handler(async ({ data }) => {
  const ctx = await getTenantContext();
