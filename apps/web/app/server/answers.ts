...
export const addAnswer = createServerFn({ method: 'POST' })
  .validator(
    (data: { questionPattern: string; category: string; answer: string; context?: string }) => data,
  )
  .handler(async ({ data }) => {
    const ctx = await getTenantContext();
    const candidate = await getCurrentCandidate(ctx);
...
export const deleteAnswer = createServerFn({ method: 'POST' })
  .validator((data: { answerId: string }) => data)
  .handler(async ({ data }) => {
    const ctx = await getTenantContext();
    const candidate = await getCurrentCandidate(ctx);
    ...
    await db
      .delete(answerBank)
      .where(and(eq(answerBank.id, data.answerId), eq(answerBank.candidateId, candidate.id)));
