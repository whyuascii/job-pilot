...
import { coverLetters, candidates, jobs, jobScores, tailoredResumes } from '@job-pilot/db/schema';
...
router.post('/generate', async (req, res, next) => {
  try {
    ...
    const existingScore = await db.query.jobScores.findFirst({ where: and(eq(jobScores.jobId, jobId), eq(jobScores.candidateId, candidate.id)) });
...
${JSON.stringify({
  ...
  experience: profile.experience.slice(0, 5).map((e: any) => ({ company: e.company, title: e.title, description: e.description, skills: e.skills })),
}, null, 2)}${scoreData ? `

## SCORE ALIGNMENT
${JSON.stringify(scoreData, null, 2)}` : ''}${tailored ? `
...
Generate a tailored cover letter.`;
    ...
    const message = await client.messages.create({ model: MODEL, max_tokens: 2048, system: COVER_LETTER_PROMPT, messages: [{ role: 'user', content: userContent }] });
    ...
    await recordLLMRun({ tenantId: ctx.tenantId, candidateId: candidate.id, jobId, purpose: 'generate_cover_letter', model: MODEL, inputTokens, outputTokens, latencyMs: Date.now() - startTime, success: true });

    // Check for existing cover letter to increment version
    const existing = await db.query.coverLetters.findFirst({ where: and(eq(coverLetters.jobId, jobId), eq(coverLetters.candidateId, candidate.id), eq(coverLetters.tenantId, ctx.tenantId)), orderBy: [desc(coverLetters.version)] });
    const nextVersion = existing ? existing.version + 1 : 1;

    const [saved] = await db.insert(coverLetters).values({
      id: createId(),
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const letter = await db.query.coverLetters.findFirst({
      where: and(eq(coverLetters.jobId, jobId), eq(coverLetters.candidateId, candidate.id), eq(coverLetters.tenantId, ctx.tenantId)),
