...
router.get('/funnel', async (_req, res, next) => {
  try {
    ...
    const funnelStages = allStages
      .filter(
        (stage) =>
          (statusMap[stage] ?? 0) > 0 || (STAGE_ORDER as readonly string[]).includes(stage),
      )
      .map((stage) => ({
        stage,
    ...
    for (const row of outcomeRows) {
      if (!outcomesByApp[row.applicationId]) outcomesByApp[row.applicationId] = [];
      outcomesByApp[row.applicationId].push({ stage: row.stage, occurredAt: row.occurredAt });
    }
    ...
    const averageTimeInStage = allStages
      .filter((s) => stageDurations[s]?.length)
      .map((s) => ({
        stage: s,
        avgDays:
          Math.round(
            (stageDurations[s].reduce((a, b) => a + b, 0) / stageDurations[s].length) * 10,
          ) / 10,
      }));
    const companyStats = await db
      .select({ company: jobs.company, totalCount: count() })
      .from(applications)
    ...
    const companyAdvanced = await db
      .select({ company: jobs.company, advancedCount: count() })
      .from(applications)
      .innerJoin(jobs, eq(applications.jobId, jobs.id))
      .where(
        and(
    ...
    const topCompanies = companyStats.map((row) => ({
      company: row.company,
...
router.get('/score-correlation', async (_req, res, next) => {
  try {
    ...
    function avgDim(
      ...
      field: 'fitBreakdown' | 'competitivenessBreakdown',
      key: string,
    ): number {
      if (!rows.length) return 0;
      const vals = rows.map((r) => (r[field] as any)?.[key] ?? 0);
      return Math.round((vals.reduce((a, b) => a + b, 0) / vals.length) * 10) / 10;
    }
    res.json({
      fitDimensions: fitKeys.map(({ key, label }) => {
        const allAvg = avgDim(allAppScores, 'fitBreakdown', key);
        const sAvg = avgDim(successfulApps, 'fitBreakdown', key);
        return {
          dimension: key,
          label,
          successfulAvg: sAvg,
          allAvg,
          delta: Math.round((sAvg - allAvg) * 10) / 10,
        };
      }),
      competitivenessDimensions: compKeys.map(({ key, label }) => {
        const allAvg = avgDim(allAppScores, 'competitivenessBreakdown', key);
        const sAvg = avgDim(successfulApps, 'competitivenessBreakdown', key);
        return {
          dimension: key,
          label,
          successfulAvg: sAvg,
...
router.get('/source-effectiveness', async (_req, res, next) => {
  try {
    ...
    const companyTotal = await db
      .select({ company: jobs.company, total: count() })
      .from(applications)
      .innerJoin(jobs, eq(applications.jobId, jobs.id))
      .where(eq(applications.tenantId, ctx.tenantId))
      .groupBy(jobs.company);
    ...
    const companyAdvanced = await db
      .select({ company: jobs.company, advanced: count() })
      .from(applications)
      .innerJoin(jobs, eq(applications.jobId, jobs.id))
      .where(
        and(
    ...
    const topCompaniesByResponse = companyTotal
      .map((row) => ({
        company: row.company,
    ...
    const sourceJobCounts = await db
      .select({ sourceId: jobs.sourceId, sourceName: jobSources.name, jobCount: count() })
      .from(jobs)
    ...
    const sourceAppCounts = await db
      .select({ sourceId: jobs.sourceId, appCount: count() })
      .from(applications)
      .innerJoin(jobs, eq(applications.jobId, jobs.id))
    ...
    const sourceInterviewCounts = await db
      .select({ sourceId: jobs.sourceId, interviewCount: count() })
      .from(applications)
      .innerJoin(jobs, eq(applications.jobId, jobs.id))
      .where(
        and(
    ...
    for (const r of sourceAppCounts) if (r.sourceId) appMap[r.sourceId] = r.appCount;
    const intMap: Record<string, number> = {};
    for (const r of sourceInterviewCounts) if (r.sourceId) intMap[r.sourceId] = r.interviewCount;
    const scoreMap: Record<string, number> = {};
    for (const r of sourceAvgScores) if (r.sourceId) scoreMap[r.sourceId] = Number(r.avgScore) || 0;
    const sourceMetrics = sourceJobCounts
      .filter((r) => r.sourceId)
      .map((r) => {
        const sid = r.sourceId!;
        const ac = appMap[sid] ?? 0;
        const ic = intMap[sid] ?? 0;
        return {
          ...
          conversionRate: ac > 0 ? Math.round((ic / ac) * 100) : 0,
        };
      })
...
router.get('/resume-variants', async (_req, res, next) => {
  try {
    ...
    const variants = rows.map((r) => {
      const status = r.applicationStatus ?? 'no_application';
      return {
        resumeId: r.resumeId,
        jobTitle: r.jobTitle,
        company: r.company,
        ...
        isSuccessful: successStatuses.includes(status),
    ...
    res.json({
      variants,
      summary: {
        totalVariants: withApps.length,
        successfulVariants: successful.length,
        successRate:
          withApps.length > 0 ? Math.round((successful.length / withApps.length) * 100) : 0,
      },
    });
  } catch (e) {
    next(e);
