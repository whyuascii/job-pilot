...
export const skills = pgTable('skills', {
  ...
  index('skills_candidate_id_idx').on(table.candidateId),
]);

export const experienceBlocks = pgTable('experience_blocks', {
  id: text('id').primaryKey().$defaultFn(createId),
  candidateId: text('candidate_id').notNull().references(() => candidates.id, { onDelete: 'cascade' }),
  ...
  index('experience_blocks_candidate_id_idx').on(table.candidateId),
]);
...
export const resumes = pgTable('resumes', {
  ...
  index('resumes_candidate_id_tenant_id_idx').on(table.candidateId, table.tenantId),
]);
